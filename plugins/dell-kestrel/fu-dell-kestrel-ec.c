...
fu_dell_kestrel_ec_is_chunk_supported(FuDevice *device, FuDellKestrelEcDevType dev_type)
{
	...
	case FU_DELL_KESTREL_EC_DEV_TYPE_PD:
		...
		guint8 chunk_support =
		    fu_struct_dell_kestrel_dock_data_get_chunk_support(self->dock_data);

		return (chunk_support & (1 << 1));
	default:
		return TRUE;
	}
}
...
fu_dell_kestrel_ec_own_dock(FuDellKestrelEc *self, gboolean lock, GError **error)
{
	...
	fu_byte_array_append_uint16(req, bitmask, G_LITTLE_ENDIAN);
